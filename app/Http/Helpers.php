...
if (! function_exists('relatedProductIds')) {
    function relatedProductIds($relatedProductIds){
        return \App\Product::whereIn('id', $relatedProductIds)->limit(10)->get();
    }
}

if (! function_exists('similarProductIds')) {
    function similarProductIds($similarProductIds){
        return \App\Product::whereIn('id', $similarProductIds)->limit(10)->get();
    }
}
...
function getComponentProductAddToCart($component_id){
    $cartItemData = [
        'id' => null,
        'quantity' => null,
        'price' => null,
...
function getComponentProductAddToSavePc($save_pc_id,$component_id){
    $cartItemData = [
        'id' => null,
        'quantity' => null,
        'price' => null,
    ...
    $save_pc_product_info = \App\SavePcDetail::where('save_pc_id',$save_pc_id)->where('component_id',$component_id)->first();
    if(!empty($save_pc_product_info)){
        //$product = \App\Product::find($cartItem['id']);
        $cartItemData['id']=$save_pc_product_info->id;
        $cartItemData['quantity']=$save_pc_product_info->quantity;
        $cartItemData['price']=$save_pc_product_info->price;
        $cartItemData['product_id']=$save_pc_product_info->product_id;
        //$cartItemData['key']=$key;
    }
...
if (! function_exists('category')) {
    function category()
    {
        return \App\Category::orderBy('order_sequence','asc')->get();
    }
}
...
if (! function_exists('general_settings')) {
    ...
    {
        return \App\GeneralSetting::find(1);
    }
}
...
if (! function_exists('checkRoute')) {
    function checkRoute($str)
    {
        ...
        $product = \App\Product::where('slug',$str)->first();
        $brand = \App\Brand::where('slug',$str)->first();
        $category = \App\Category::where('slug',$str)->first();
        $subcategory = \App\SubCategory::where('slug',$str)->first();
...
if (! function_exists('checkRouteCatSubCat')) {
    function checkRouteCatSubCat($cat_slug,$sub_cat_slug)
    {
        $data = [
            'found' => 0,
            'type' => '',
        ...
        $category_id = getCategoryIdByCategorySlug($cat_slug);
        $subcategory = \App\SubCategory::where('category_id',$category_id)->where('slug',$sub_cat_slug)->first();
        if($subcategory){
            $data = [
                'found' => 1,
                'type' => 'subcategory',
...
if (! function_exists('checkRouteCatSubCatBrand')) {
    function checkRouteCatSubCatBrand($cat_slug,$sub_cat_slug,$brand_slug)
    {
        $data = [
            'found' => 0,
            'type' => '',
...
if (! function_exists('getCategoryIdBySubCategoryId')) {
    function getCategoryIdBySubCategoryId($subsubcategory_id)
    {
        return \Illuminate\Support\Facades\DB::table('categories')
            ->join('sub_categories','categories.id','sub_categories.category_id')
            ->where('sub_categories.id',$subsubcategory_id)
            ->pluck('categories.id')
            ->first();
    }
}

if (! function_exists('getCategoryIdByCategorySlug')) {
    function getCategoryIdByCategorySlug($category_slug)
    {
        return \Illuminate\Support\Facades\DB::table('categories')
...
if (! function_exists('getProductSpecificationTitleByProductId')) {
    function getProductSpecificationTitleByProductId($id)
    {
        return \Illuminate\Support\Facades\DB::table('product_specifications')
            ->where('product_id',$id)
            ->select('title')
            ->groupBy('title')
            ->orderBy('id')
...
if (! function_exists('getProductSpecificationByProductId')) {
    function getProductSpecificationByProductId($id,$title)
    {
        return \Illuminate\Support\Facades\DB::table('product_specifications')
            ->where('product_id',$id)
            ->where('title',$title)
...
if (! function_exists('getBrandNameByBrandId')) {
    function getBrandNameByBrandId($id)
    {
...
if (! function_exists('getBrandSlugByBrandId')) {
    function getBrandSlugByBrandId($id)
    {
        return \Illuminate\Support\Facades\DB::table('brands')
            ->where('id',$id)
            ->pluck('slug')
...
if (! function_exists('getGroupByProductsByCategoryId')) {
    function getGroupByProductsByCategoryId($category_id)
    {
        return \App\Product::where('published', 1)
            ->select('brand_id')
