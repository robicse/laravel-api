...
<link type="text/css" href="{{ static_asset('frontend/css/meanmenu.min.css') }}" rel="stylesheet" media="all">
<link type="text/css" href="{{ static_asset('frontend/css/style.css') }}" rel="stylesheet" media="all">
<link type="text/css" href="{{ static_asset('frontend/css/kh-custom.css') }}" rel="stylesheet" media="all">

<link type="text/css" href="{{ static_asset('frontend/css/pc-build.css') }}" rel="stylesheet" media="all">
...
@endif
    ...
    <!-- Global site tag (gtag.js) - Google Analytics -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-4CBV5WSZ96"></script>
    <script>
        window.dataLayer = window.dataLayer || [];
        function gtag(){dataLayer.push(arguments);}
        gtag('js', new Date());

        gtag('config', 'G-4CBV5WSZ96');
    </script>
...
@endif
    ...
    <script>
        ...
        OneSignal.push(function() {
            OneSignal.init({
                appId: "d784b51b-1684-477a-8596-252981da52af",
            });
        });
    </script>
</head>
...
<script src="{{ static_asset('frontend/js/jquery.sticky-sidebar.min.js') }}"></script>
...
<script src="{{ static_asset('frontend/js/jquery.meanmenu.min.js') }}"></script>
...
<script>
    ...
    function removeTaxFromCart(){
        ...
        $.post('{{ route('cart.removeTaxFromCart') }}', {_token:'{{ csrf_token() }}', tax:tax}, function(data){
            updateNavCart();
            $('#cart-summary').html(data);
    ...
    function showPcBuildAddToCartModal(id){
        ...
        $('#addToCart').modal();
        $('.c-preloader').show();
        $.post('{{ route('cart.showPcBuildCartModal') }}', {_token:'{{ csrf_token() }}', id:id}, function(data){
            $('.c-preloader').hide();
            $('#addToCart-modal-body').html(data);
            $('.xzoom, .xzoom-gallery').xzoom({
                Xoffset: 20,
    ...
    function addToCartPcBuild(){
        if(checkAddToCartValidity()) {
            ...
            $.ajax({
                type:"POST",
                url: '{{ route('cart.addToCartPcBuild') }}',
                data: $('#option-choice-form').serializeArray(),
                success: function(data){
                    $('#addToCart-modal-body').html(null);
                    $('.c-preloader').hide();
    ...
    function addNow(id){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                updateNavCart();
                $('#cart_items_sidenav').html(parseInt($('#cart_items_sidenav').html())+1);
                window.location.replace("{{ route('pc-build') }}");
            }
        });
    }
    ...
    function getQuotation(){
        @if (Auth::check() && (Auth::user()->user_type == 'customer' || Auth::user()->user_type == 'seller'))
            if(checkAddToCartValidity()) {
                ...
                $.ajax({
                    type:"POST",
                    url: '{{ route('get.quotation') }}',
                    data: $('#option-choice-form').serializeArray(),
                    success: function(data){
                        $('.c-preloader').hide();
                        console.log(data);
                        alert('Your Quotation Successfully Sent To Your Mail');
                    }
                });
            }
            else{
